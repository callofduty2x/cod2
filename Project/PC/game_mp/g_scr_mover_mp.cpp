void (__cdecl*__cdecl ScriptEnt_GetMethod(char const * *))(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmdGetCommandTimes(float *,float *,float *)
{	UNIMPLEMENTED();
}

void __cdecl InitScriptMover(struct gentity_s *)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_NotSolid(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_Solid(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl SP_script_origin(struct gentity_s *)
{	UNIMPLEMENTED();
}

void __cdecl SP_script_brushmodel(struct gentity_s *)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_RotateVelocity(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEnt_RotateAxis(struct scr_entref_t,int)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_RotateTo(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEnt_MoveAxis(struct scr_entref_t,int)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_GravityMove(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_MoveTo(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_RotateRoll(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_RotateYaw(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_RotatePitch(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_MoveZ(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_MoveY(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl ScriptEntCmd_MoveX(struct scr_entref_t)
{	UNIMPLEMENTED();
}

void __cdecl SP_script_model(struct gentity_s *)
{	UNIMPLEMENTED();
}

void __cdecl Reached_ScriptMover(struct gentity_s *)
{	UNIMPLEMENTED();
}

void __cdecl ScriptMover_SetupMoveSpeed(struct trajectory_t *,float const * const,float,float,float,float * const,float *,float *,float *,float * const,float * const,float * const)
{	UNIMPLEMENTED();
}

void __cdecl ScriptMover_SetupMove(struct trajectory_t *,float const * const,float,float,float,float * const,float *,float *,float *,float * const,float * const,float * const)
{	UNIMPLEMENTED();
}

int __cdecl ScriptMover_Updatemove(struct trajectory_t *,float * const,float,float,float,float const * const,float const * const,float const * const)
{	UNIMPLEMENTED();
}

void __cdecl ScriptMover_RotateSpeed(struct gentity_s *,float const * const,float,float,float)
{	UNIMPLEMENTED();
}

void __cdecl ScriptMover_Rotate(struct gentity_s *,float const * const,float,float,float)
{	UNIMPLEMENTED();
}

void __cdecl ScriptMover_GravityMove(struct gentity_s *,float const * const,float)
{	UNIMPLEMENTED();
}

void __cdecl ScriptMover_Move(struct gentity_s *,float const * const,float,float,float)
{	UNIMPLEMENTED();
}

